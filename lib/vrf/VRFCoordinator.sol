...
contract VRFCoordinator is VRF, VRFRequestIDBase {
    ...
    event RandomnessRequest(bytes32 keyHash, uint256 seed, bytes32 indexed jobID, address sender, uint256 fee);
    ...
    function randomnessRequest(
        ...
        emit RandomnessRequest(_keyHash, seed, serviceAgreements[_keyHash].jobID, _sender, _feePaid);
    ...
    function fulfillRandomnessRequest(bytes memory _proof) public returns (bool) {
        ...
        VRFConsumerBase v;
        bytes memory resp = abi.encodeWithSelector(v.fulfillRandomness.selector, requestId, randomness);
