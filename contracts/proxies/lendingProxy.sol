...
contract CompoundResolver is CompoundHelpers {
     ...
     */
    function compoundDeposit(address token, uint256 amt) internal returns (uint256 _amt) {
        _amt = amt;
     ...
     */
    function compoundWithdraw(address token, uint256 amt) internal returns (uint256 _amt) {
        _amt = amt;
        address cToken = Mapping(getMappingAddr()).cTokenMapping(token);
        ...
        if (_amt == uint256(-1)) {
            TokenInterface tokenContract = TokenInterface(token);
            uint256 initialBal =
                token == getAddressETH() ? address(this).balance : tokenContract.balanceOf(address(this));
            uint256 cTknBal = cTokenContract.balanceOf(address(this));
...
contract AaveHelpers is CompoundResolver {
    ...
    // TODO - change7878
    function getWithdrawBalance(address token) public view returns (uint256 bal) {
...
contract AaveResolver is AaveHelpers {
     ...
     */
    function aaveDeposit(address token, uint256 amt) internal returns (uint256 _amt) {
        _amt = amt;
        AaveProviderInterface lendingProviderPool = AaveProviderInterface(getAaveProviderAddress());
        AaveInterface aave = AaveInterface(lendingProviderPool.getLendingPool());
    ...
    function aaveWithdraw(address token, uint256 amt) internal returns (uint256 _amt) {
        _amt = amt;
        ...
        uint256 initialBal =
            token == getAddressETH() ? address(this).balance : TokenInterface(token).balanceOf(address(this));
        atoken.redeem(_amt);
        uint256 finialBal =
            token == getAddressETH() ? address(this).balance : TokenInterface(token).balanceOf(address(this));
        uint256 withdrawnAmt = sub(finialBal, initialBal);
...
contract LendingProxy is AaveResolver {
    ...
    function deposit(
        ...
        address token,
        uint256 amount
    ) external payable returns (uint256 depositAmt) {
        if (lendingId == 1) {
    ...
    function withdraw(
        ...
        address token,
        uint256 amount
    ) external returns (uint256 withdrawAmt) {
        if (lendingId == 1) {
