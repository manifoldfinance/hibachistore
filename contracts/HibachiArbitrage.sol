...
import { KingRollSwapInterface } from "./interfaces/kingRollSwap.sol";
import { IERC20 } from "./interfaces/IERC20.sol";
import { IWETH } from "./interfaces/IWETH.sol";
import { SafeMath } from "./libraries/SafeMath.sol";

// HibachiArbitrage is the arbitrage contract that deals with arbitrage opportunities per trade
// Right now the prize pool is long DAI,ETH,USDT,USDC
contract HibachiArbitrage is IUniswapV2Callee {
    ...
    function calculateAmountForArbitrage(address token, bool IsDirectionETHToToken) public view returns (uint256) {
        ...
        (bool EthToToken, uint256 amountIn) = computeProfitMaximizingTrade(1, price, reserveETHV1, reserveTokenV1);
    ...
    function uniswapV2Call(
        ...
        uint256 amountETH;
        {
            ...
            require(
                ...
                "CS: Msg sender is not the uniswapV2 pair"
        ...
        if (amountToken > 0) {
            ...
            require(
                token.transfer(sender, amountReceived - amountRequired),
                "CS: Token transfer to original transfer failed"
        ...
        )
    {
        ...
        pair.swap(
            (pair.token0() == address(token) ? numOfTokensToBeTraded : 0),
            (pair.token0() == address(token) ? 0 : numOfTokensToBeTraded),
            address(this),
            abi.encode(slippageParam)
        );
        // revert if we didnt get profit
        require(address(this).balance > EthBeforeArb, "CS: Hibachi shop should have profit to split");
        uint256 profit = address(this).balance - EthBeforeArb;

        if (withHibachi) {
            (leftProfit, numHibachi) = KingRollSwapInterface(governance.kingRollSwap()).swapEthToDai{ value: (profit) }(
        ...
        numTokensObtained = numTokensObtained + exchangeV1.ethToTokenSwapInput{ value: leftProfit }(1, uint256(-1));
        require(
            IERC20(token).transfer(msg.sender, numTokensObtained),
            "CS: Transfer tokens to original swapper failed"
        );
    }
        ...
        )
    {
        ...
        numTokensObtained = exchangeV1.ethToTokenSwapInput{ value: msg.value }(minTokens, uint256(-1));
        leftProfit;
        if (WithArb) {
            (numTokensObtained, leftProfit, numHibachi) = _ethToTokenArbs(
                numTokensObtained,
                token,
                exchangeV1,
                slippageParam,
            ...
            if (withHibachi) {
                require(IERC20(token).approve(governance.kingRollSwap(), numTokensObtained), "approve not successfull");
                (numTokensObtained, numHibachi) = KingRollSwapInterface(governance.kingRollSwap()).swapTokenToDai(
                    msg.sender, // Sender of the swap
                    msg.sender, // Buy hibachis for addresss
                    token, // token address
    ...
    function _TokenToEthHibachi(
        ...
        uint256 profit,
        bool withHibachi
    ) internal returns (uint256 leftProfit, uint256 numHibachi) {
        if (withHibachi) {
            require(IERC20(token).approve(governance.kingRollSwap(), profit), "approve not successfull");
            (leftProfit, numHibachi) = KingRollSwapInterface(governance.kingRollSwap()).swapTokenToDai(
                msg.sender,
                msg.sender,
                token,
                profit,
        ...
        )
    {
        ...
        // finally we execute a arb to reduce slippage
        pair.swap(
            (pair.token0() == address(token) ? 0 : numOfEthToBeArbedWith),
            (pair.token0() == address(token) ? numOfEthToBeArbedWith : 0),
            address(this),
            abi.encode(slippageParam)
        );
        ...
        // revert if we didnt get profit
        require(
            IERC20(token).balanceOf(address(this)) > TokensBeforeArb,
            "CS: Hibachi shop should have profit to split"
        );
        uint256 profit = IERC20(token).balanceOf(address(this)) - TokensBeforeArb;
        ...
        numEthObtained =
            numEthObtained +
            IUniswapV1Exchange(factoryV1.getExchange(token)).tokenToEthSwapInput(leftProfit, 1, deadline);
        ...
        )
    {
        ...
        leftProfit;
        if (WithArb) {
            (numEthObtained, leftProfit, numHibachi) = _TokenToEthArbs(
                numEthObtained,
                token,
                exchangeV1,
                slippageParam,
            ...
            if (withHibachi) {
                (numEthObtained, numHibachi) = KingRollSwapInterface(governance.kingRollSwap()).swapEthToDai{
                    value: numEthObtained
                }(msg.sender, msg.sender, numEthObtained, true, true);
            }
        }
    }

    event Report(uint256 tokenBrought, uint256 profit, uint256 hibachisBrought);
    ...
    function swap(
        ...
        returns (
            ...
            uint256 profit,
            uint256 hibachisBrought
        )
    {
        if (buyAddr != ethAddr) {
            require(sellAmt == msg.value, "msg.value is not same");
            (tokenBrought, profit, hibachisBrought) = EthToTokenSwap(
                buyAddr,
                deadline,
                minBuyAmt,
                slippage,
            ...
            (tokenBrought, profit, hibachisBrought) = TokenToEthSwap(
                sellAddr,
                sellAmt,
                deadline,
                minBuyAmt,
        ...
        emit Report(tokenBrought, profit, hibachisBrought);
    }
    ...
    function approveTokenToHibachiArbitrage(address token, uint256 amount) external {
        ...
        )
    {
        ...
        if (EtoT) {
            uint256 _t1 = IUniswapV1Exchange(exchangeV1).getEthToTokenInputPrice(amountSold);
            ethBalanceV1 = ethBalanceV1.add(amountSold);
            tokenBalaceV1 = tokenBalaceV1.sub(_t1);
        } else {
            uint256 _e1 = IUniswapV1Exchange(exchangeV1).getTokenToEthInputPrice(amountSold);
